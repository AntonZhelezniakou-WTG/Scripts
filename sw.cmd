...
:: ---------------------------------------------------------
:: 7) Rebase the current branch on top of master, except for master.
:: ---------------------------------------------------------
:rebase_master
if /I "%BRANCH%"=="master" (
	rem No rebase for master branch.
) else (
	echo Rebasing branch "%BRANCH%" onto master...
	git rebase master --no-edit
	if errorlevel 1 (
		echo Rebase conflicts detected. Gathering conflict files...
		set "CONFLICT_FILES="
		...
		:WAIT_FOR_RESOLVE
		choice /c RA /m "Conflicts detected. Type R to retry conflict check, A to abort the rebase."
		if errorlevel 2 (
		...
		if errorlevel 1 (
			echo Error: Rebase continue operation failed.
			goto :error_exit
		)
